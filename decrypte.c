...
void generer_sous_cle_D(char* cle, sous_cle* sous_cles) {
	...
	for (i=1; i<17; i++) {
		...
		decalage = bytes_de_decalage_par_cle_D[i];
		if (decalage == 1){
			octet_decalage = 0x80;
		} else {
			octet_decalage = 0xC0;
...
char* decrypter(char* message) {
	long taille_message;
	int padding;
	char* message_decrypted = (char*)malloc((sizeof(message)/sizeof(char))*sizeof(char));

	FILE *cle_file;
	cle_file = fopen("keyfile.txt", "r"); // option "r" pour lire dans un fichier texte
	...
	while(i<total_block) {
			...
			memset(block_decrypted, 0, 8);

			for (j=0; j<64; j++) {
				decalage = table_permutation_init_block_D[j]; // ex : decalage = 58 ;
				octet_decalage = 0x80 >> ((decalage - 1)%8); // les 8 premiers bits se decale de 1 vers la droite, octet_decalage = 01000000
				octet_decalage &= block[(decalage - 1)/8]; // octet_decalage = 01000000 & 01100001 ('c') = 01000000
			...
			for (k=1; k<=16; k++) {
				...
				// on suit l'ordre inverse pr le decryptage
				cycle = k-17;

				for (j=0; j<6; j++) {
					droite_etendue[j] ^= sous_cles[cycle].cle[j]; // fonction xor entre la partie droite et la derniere cle pr k = 1
				}
				...
				droite_etendue_substituee[0] |= ((char)S1_D[ligne*16+colonne] << 4);
				...
				droite_etendue_substituee[0] |= (char)S2_D[ligne*16+colonne];
				...
				droite_etendue_substituee[1] |= ((char)S3_D[ligne*16+colonne] << 4);
				...
				droite_etendue_substituee[1] |= (char)S4_D[ligne*16+colonne];
				...
				droite_etendue_substituee[2] |= ((char)S5_D[ligne*16+colonne] << 4);
				...
				droite_etendue_substituee[2] |= (char)S6_D[ligne*16+colonne];
				...
				droite_etendue_substituee[3] |= ((char)S7_D[ligne*16+colonne] << 4);
				...
				droite_etendue_substituee[3] |= (char)S8_D[ligne*16+colonne];

				for (j=0; j<4; j++) {
					d[j] = 0;
				}
				// fonction xor entre la partie gauche initiale et la nouvelle partie droite

				for (j=0; j<32; j++) {
					decalage = table_permutation_block_partie_droite_D[j];
					octet_decalage = 0x80 >> ((decalage - 1)%8);
			...
			for (i=0; i<64; i++) {
				...
				block_decrypted[j/8] |= (octet_decalage >> i%8);
			}

			memset(block, 0, 8);

			memcpy(message_decrypted +i*8,block_decrypted,8);
	}
	// enfin on libere l'espace memoire des 3 tableaux
	free(cle);
	free(block);
	free(block_decrypted);
